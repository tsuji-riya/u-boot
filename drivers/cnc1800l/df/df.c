...
#define CONFIG_TVOUT_CVBS
#ifdef CONFIG_TVOUT_CVBS
#define VIDEO2PITCH 2048
void DFVideoEna(int VideoId, int IsEna, int v_w, int v_h, unsigned int yuv_addr)
{
	...
	switch(VideoId)
	{
		...
		case 1:
			...
			reg_video2->df_video_x_position_reg.val = 0x02d00000;
			reg_video2->df_video_y_position_reg.val = 0x02400000;
			reg_video2->df_video_yt_addr_reg.val = yuv_addr;
			reg_video2->df_video_ct_addr_reg.val = yuv_addr + (VIDEO2PITCH * v_h);
			reg_video2->df_video_yb_addr_reg.val = yuv_addr + 0x800;
			reg_video2->df_video_cb_addr_reg.val = yuv_addr + (VIDEO2PITCH * v_h) + 0x800;
			reg_video2->df_video_src_ctrl_reg.val = 0x00000100;	
			df_write(dfreg_video1[1], reg_video2->df_video_x_position_reg.val);
			df_write(dfreg_video1[2], reg_video2->df_video_y_position_reg.val);
			df_write(dfreg_video1[3], reg_video2->df_video_yt_addr_reg.val);
			df_write(dfreg_video1[4], reg_video2->df_video_ct_addr_reg.val);
			df_write(dfreg_video1[5], reg_video2->df_video_yb_addr_reg.val);
			df_write(dfreg_video1[6], reg_video2->df_video_cb_addr_reg.val);
			df_write(dfreg_video1[7], reg_video2->df_video_src_ctrl_reg.val);
...
void sdoutput(TVOUT_MODE mode, unsigned int addr)
{
	TVOUT_MODE tve1_mode;
	int cvbs_w = 720, cvbs_h;

	switch (mode){
		case TVOUT_MODE_SECAM:
			tve1_mode = TVOUT_MODE_SECAM;
			cvbs_h = 576;
			break;
		case TVOUT_MODE_PAL_CN:
			tve1_mode = TVOUT_MODE_PAL_CN;
			cvbs_h = 576;
			break;
		case TVOUT_MODE_PAL_N:
			tve1_mode = TVOUT_MODE_PAL_N;
			cvbs_h = 576;
			break;
		case TVOUT_MODE_PAL_M:
			tve1_mode = TVOUT_MODE_PAL_M;
			cvbs_h = 480;
			break;
		case TVOUT_MODE_576I:
		case TVOUT_MODE_576P:
	...
	clock_dispset_clockmode0(1, tve_2_clock[tve1_mode]);
	udelay(100);
	
	InitTVE1Raw(tve1_mode, __Is_ColorBar);
	udelay(100);

	__df_update_start();
	DFSetOutIFVideoFmt(1, tve_2_df[tve1_mode]);
	__df_update_end();
	udelay(100);

	__df_update_start();
	DFVideoEna(1, 1, cvbs_w, cvbs_h,addr);
	__df_update_end();	
...
extern int do_call_external_proc (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[]);
...
#if 1
	if((mode < TVOUT_RGB_1366X768_60FPS)&&(per_mode !=mode)){
		char *hdmi_argv[4]={0};
		char mode_str[5]={0};
		sprintf(mode_str,"%d",mode);
		hdmi_argv[0]="call_external_proc";
		hdmi_argv[1]="HDMI";
		hdmi_argv[2]=mode_str;
